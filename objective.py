...
def nt_xent(embedding1, embedding2, temperature=0.1, num_replicas=None):
    ...
    if num_replicas > 1:
        ...
        cur_replica_labels = F.one_hot(labels, full_batch_size)
        other_replica_labels = F.one_hot(labels, full_batch_size * 2)
    else:
        embedding1_full = embedding1
        embedding2_full = embedding2
        cur_replica_labels = F.one_hot(torch.arange(batch_size), batch_size)
        other_replica_labels = F.one_hot(torch.arange(batch_size), batch_size * 2)
